...
class FragmentDialogQuestion : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        arguments?.getInt(POSITIVE_BUTTON_TEXT)?.let {
            builder.setPositiveButton(if (it == 0) R.string.dialog_btn_ok else it) { _, _ ->
                val bundle = Bundle()
                arguments?.getInt(ActivityMain.DIALOG_TYPE)?.let {
                    bundle.putInt(ActivityMain.DIALOG_TYPE, it)
                }

                requireActivity().supportFragmentManager.setFragmentResult(
                    ActivityMain.DIALOG_REQUEST_KEY, bundle
                )
    ...
    companion object {
        ...
        ): FragmentDialogQuestion {
            val args = Bundle()

            args.putInt(ActivityMain.DIALOG_TYPE, type)

            if (titleId != null) args.putInt(TITLE, titleId)

            args.putInt(MESSAGE, messageId)

            if (positiveButtonTextId != null) args.putInt(
                POSITIVE_BUTTON_TEXT,
                positiveButtonTextId
            )
            if (negativeButtonTextId != null) args.putInt(
        ...
        fun show(
            ...
            val dialogQuestion = newInstance(
                type,
                titleId,
                messageId,
                positiveButtonTextId,
